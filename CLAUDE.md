...
- GitHub renders several merge box variants and only some contain `section[aria-label="Reviews"]`. It is **absent on draft PRs, closed PRs, and merged PRs**. ⚠️ **Absence tracks the PR's state, not its stack position** — a stacked *child* renders a full Reviews section, blocking line and "Merging is blocked" like any other PR. An earlier version of this doc claimed stacked PRs always lack the section; it was generalising from a single PR that turned out to be **closed**, not merely stacked. 0.8.0 already accommodated its absence for *positioning*; the state readers did not, so the header silently fell back to GitHub's generic "N approvals required by reviewers with write access".
...
- **Drafts are now the main consumer of the sidebar fallback**, since stacked PRs generally have a Reviews section after all. The blocking line *is* rendered on drafts, so absence of it there means satisfied, not unrendered. ⚠️ **This is inherited from before the fallback existed and has not been observed under the current readers** — drafts are simultaneously the least-tested path here and the one with no merge box signal to fall back on. Verify against a real draft before relying on any of it.
- **Known hole — the sidebar can fail to load.** It intermittently renders "There was an error while loading. Please reload this page." in place of its content (observed twice while testing this section). That empties `parseReviewerRows()` and silently drops the shield signal, and there is no guard for it. Both observed cases were carried by their Reviews section, so nothing broke — but a draft in that state has no Reviews section to fall back on and would lose its owner requirement outright.
