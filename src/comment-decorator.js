...
const decorateDraftWriteTabs = async () => {
  const writeTabSelectors = [
    '.CommentBox-header .write-tab:not(.ghco-processed)', // Old UI
    '[class*="prc-TabNav"] button[role="tab"]:first-of-type:not(.ghco-processed)', // New UI - Write is always first
  ];
  const allWriteTabs = document.querySelectorAll(writeTabSelectors.join(', '));

  if (allWriteTabs.length === 0) {
    return;
  }
  ...
  allWriteTabs.forEach((writeTab) => {
    if (writeTab.classList.contains('ghco-processed')) {
      return;
    }
    writeTab.classList.add('ghco-processed');
